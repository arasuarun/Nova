...
impl<E: Engine> R1CSShape<E> {
  ...
  pub(crate) fn is_regular_shape(&self) -> bool {
    ...
    let vars_valid = (self.num_vars).next_power_of_two()
      == (self.num_vars);
  ...
  pub fn is_sat_relaxed(
    ...
    let res_comm = {
      let (comm_W, comm_E) = rayon::join(
        || (CE::<E>::commit(ck, &W_split.0), CE::<E>::commit(ck, &W_split.1)),
        || CE::<E>::commit(ck, &W.E),
      );
  ...
  pub fn is_sat(
    ...
    let W_split = W.W.split_at(2);
    let res_comm = U.comm_W == (CE::<E>::commit(ck, &W_split.0), CE::<E>::commit(ck, &W_split.1));
...
impl<E: Engine> R1CSWitness<E> {
  ...
  pub fn commit(&self, ck: &CommitmentKey<E>) -> (Commitment<E>, Commitment<E>) {
    let W_split = self.W.split_at(2);
    (
      CE::<E>::commit(ck, &W_split.0),
      CE::<E>::commit(ck, &W_split.1),
    )
  }
}
...
impl<E: Engine> RelaxedR1CSWitness<E> {
  ...
  pub fn pad(&self, S: &R1CSShape<E>) -> RelaxedR1CSWitness<E> {
    ...
    W.extend(vec![E::Scalar::ZERO; S.num_vars]);
